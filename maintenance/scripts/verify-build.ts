...
// --- llms.txt ------------------------------------------------------------
// Generated by og-data-gen.ts from the same database as the sitemap, and
// gitignored, so a build that skipped og:generate would ship the site without
// it — or, worse, with an empty file. Nothing in the app reads it, so its
// absence is invisible from the inside; assert it here instead.
const llmsPath = resolve(DIST, "llms.txt");
if (!existsSync(llmsPath)) {
	fail("missing llms.txt — og:generate did not run before vite build");
} else {
	const llms = readFileSync(llmsPath, "utf-8");
	if (llms.trim().length === 0) {
		fail("llms.txt is empty");
	} else if (!llms.includes(`${nameIds.length} names`)) {
		fail(
			`llms.txt does not state the current scale (${nameIds.length} names) — ` +
				`the counts must come from the database, not be hardcoded`,
		);
	} else if (!llms.includes("/sitemap.xml")) {
		fail("llms.txt does not point at the sitemap");
	}
}
...
console.log(
	`✅ Build verified: ${namePages} name pages, ${speciesPages} species pages, ` +
		...
		`llms.txt present with live counts, ` +
